...
App.run(["$rootScope", "$state", "$stateParams",  '$window', '$templateCache', function ($rootScope, $state, $stateParams, $window, $templateCache) {
  ...
  $rootScope.app = {
    name: 'CenoCloud +',
    description: 'CenoCloud Gateway Console',
    keywords: "CenoCloud,console,gateway",
  ...
  $rootScope.user = {
    name:     'XXX',
    job:      'JOB',
    picture:  'app/img/user/avatar.jpg'
  };

  // API 请求域
  $rootScope.api = "http://www.cenocloud.com:8080/api/api";
...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
  ...
  $urlRouterProvider.otherwise('/app/apinote');
  ...
  $stateProvider
    ...
    .state('app.apinote', {
        url: '/apinote',
        title: 'API Document',
        templateUrl: helper.basepath('apinote.html'),
    })
    // Mock 列表
    .state('app.mock', {
        url: '/mock',
        title: 'API Mock',
        templateUrl: helper.basepath('mock.html')
    })
    // Origin 控制
    .state('app.origin', {
        url: '/origin',
        title: 'Origin Control',
        templateUrl: helper.basepath('origin.html'),
    })
    // 防火墙
    .state('app.iptable', {
        url: '/iptable',
        title: 'IP Table',
        templateUrl: helper.basepath('iptable.html'),
    })
...
App.controller('FlotChartController', ['$scope', 'ChartData', '$timeout', function($scope, ChartData, $timeout) {
    ...
    $scope.splineOptions = {
        ...
        tooltip: true,
        tooltipOpts: {
            content: function(label, x, y) { return x + ' : ' + y + '次'; }
        },
        xaxis: {
            tickColor: '#fcfcfc',
            mode: 'categories'
        ...
        yaxis: {
            min: 0,
            // max: 10000, // optional: use it for a clear represetation
            tickColor: '#eee',
...
App.controller('AppController',
  ...
  function($rootScope, $scope, $state, $translate, $window, $localStorage, $timeout, toggle, colors, browser, cfpLoadingBar) {
    ...
    $rootScope.pageTitle = function() {
      var title = ($rootScope.currTitle || $rootScope.app.description) + ' | ' + $rootScope.app.name;
